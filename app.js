...
(() => {
  ...
  const config = window.BilingvoConfig || {};
  const assetBase = new URL(config.assetBase || root.dataset.assetBase || './assets/', document.baseURI);
  ...
  $$('[data-lead-form]').forEach(form=>form.addEventListener('submit',async event=>{
    ...
    try{
      ...
      const response=await fetch(config.formEndpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...fields,form:form.dataset.leadForm}),signal:controller.signal});
