...
(() => {
  ...
  const size=500,radius=222,cx=250,cy=250;
  ...
  function center(city){
    selected=cities.indexOf(city);lastInteraction=performance.now();inertia=0;
    const delta=((city.lon-longitude+540)%360)-180;
    flight={fromLon:longitude,fromLat:latitude,toLon:longitude+delta,toLat:city.lat,start:performance.now(),duration:reduce.matches?0:1250};
    cityStatus.textContent=city.name;cityStatus.dataset.city=city.name;
  ...
  function draw(now){
    ...
    if(flight){const t=flight.duration?Math.min(1,(now-flight.start)/flight.duration):1,k=t*t*(3-2*t);longitude=flight.fromLon+(flight.toLon-flight.fromLon)*k;latitude=flight.fromLat+(flight.toLat-flight.fromLat)*k;if(t===1)flight=null;}
    else if(!drag&&!reduce.matches){if(Math.abs(inertia)>.001){longitude+=inertia*dt;inertia*=.94;}else if(now-lastInteraction>6000)longitude+=dt*.0015;}
    ...
    [0,1,2,3].forEach((id,j)=>{const target=cities[id].v;let started=false;ctx.beginPath();for(let i=0;i<=80;i++){const t=i/80;let v=home.map((n,k)=>n*(1-t)+target[k]*t);const length=Math.hypot(...v);v=v.map(n=>n/length);const p=project(v);if(p[2]>0){const x=cx+p[0]*radius,y=cy-p[1]*radius;if(!started){ctx.moveTo(x,y);started=true;}else ctx.lineTo(x,y);}else started=false;}ctx.strokeStyle=id===selected?'#2b8a65aa':'#508d7355';ctx.lineWidth=1.1;ctx.setLineDash([2,5]);ctx.lineDashOffset=reduce.matches?0:-now/150;ctx.stroke();ctx.setLineDash([]);});
    hits=[];
    cities.map((city,index)=>({city,index,p:project(city.v)})).filter(o=>o.p[2]>.03).sort((a,b)=>a.p[2]-b.p[2]).forEach(({city,index,p})=>{
      ...
      ctx.beginPath();ctx.arc(x,y,index===selected?3:2.2,0,Math.PI*2);ctx.fillStyle=index===selected?'#075b3c':'#25895c';ctx.fill();
      if(index===selected){ctx.beginPath();ctx.arc(x,y,s+4+(reduce.matches?0:Math.sin(now/350)*1.5),0,Math.PI*2);ctx.strokeStyle='#318b6566';ctx.lineWidth=1.1;ctx.stroke();}
    });
